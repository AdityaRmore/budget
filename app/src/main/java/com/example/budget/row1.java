...
public class row1 extends AppCompatActivity {

    RecyclerView jan_recyc ;
    TextView month_text;

    FirebaseRecyclerOptions<Car> options;
    FirebaseRecyclerAdapter<Car, MyVieHolder> madapter;
    DatabaseReference  jan_dataRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        jan_recyc=findViewById(R.id.jan_recyc);
        month_text=findViewById(R.id.textviewof_month);

        LinearLayoutManager linearLayoutManager= new LinearLayoutManager(getApplicationContext());
        jan_recyc.setLayoutManager(linearLayoutManager);
        linearLayoutManager.setReverseLayout(true);
        linearLayoutManager.setStackFromEnd(true);
        jan_recyc.setHasFixedSize(true);



        FirebaseUser user= FirebaseAuth.getInstance().getCurrentUser();
        final String favuser=user.getUid();
        jan_dataRef= FirebaseDatabase.getInstance().getReference().child("Jan").child(favuser);//also called  CAR
    ...
    private void jan_data() {
        ...
        options = new FirebaseRecyclerOptions.Builder<Car>().setQuery(jan_dataRef, Car.class).build();
